...
interface Store {
  ...
  sendMessage(message: string): { content: string };
  receiveMessage(encryptedContent: string): string;
...
const store = reactive<Store>({
  ...
  initSocket() {
    if (!this.socket) {
      try {
        ...
        const socketPath =
          (window as any).__APP_ENV__?.SOCKET_PATH || "/socket.io";
        ...
        socket.on("connect_error", (err: Error) => {
          ...
          this.connectionError =
            "Failed to connect to chat server: " + err.message;
        ...
        socket.on("connect", () => {
          ...
          if (storedUsername) {
            console.log(
              "Found stored username, attempting to reconnect:",
              storedUsername
            );
        ...
        socket.on(
          "encryption_key",
          (keys: { key: string; masterKey: string }) => {
  ...
  receiveMessage(encryptedContent: string): string {
    return this.receiveMessage(encryptedContent);
  },
